...
pub(super) async fn serve(
    ...
    futures::future::try_join(
        ...
        async {
            type Service =
                aggregated_discovery_service_server::AggregatedDiscoveryServiceServer<Server>;
            loop {
            ...
            match item {
                Either::Left(request) => {
                        ...
                        && let Some((version, endpoints)) = rx.borrow().clone()
                    {
                        ...
                        && let Some((version, endpoints)) = rx.borrow().clone()
                    {
                        let (_, route_configuration_next) = generator
                            .generate(&endpoints)
                            .map_err(|e| tonic::Status::internal(e.to_string()))?;
                ...
                Either::Right(Some((version, endpoints))) => {
                    let (clusters_next, route_configuration_next) = generator
                        .generate(&endpoints)
                        .map_err(|e| tonic::Status::internal(e.to_string()))?;
                    if clusters
                        .as_ref()
...
impl Generator {
    fn generate(
...

        {
            ...
            let mut route = self.template_route.clone().unwrap_or_default();
